...
trait SubscriptionHistoryTrait
{
    ...
    public function paymentApprove($historyId, $forceCarry = false, $activeNow = false)
    {
        ...
        if ($activePackage) {
            ...
            if (session('admin_customer') == true && $activeNow) {
        ...
        $subscriptionHistory->new_word_balance      = $subscriptionPackage->total_words_per_month;
        $subscriptionHistory->new_image_balance     = $subscriptionPackage->total_images_per_month;
        $subscriptionHistory->new_s2t_balance       = $subscriptionPackage->total_speech_to_text_per_month;

        $subscriptionHistory->this_month_available_words   = (int) $subscriptionPackage->total_words_per_month + $carriedWords;
        $subscriptionHistory->this_month_available_images  = (int) $subscriptionPackage->total_images_per_month + $carriedImages;
        $subscriptionHistory->this_month_available_s2t     = (int) $subscriptionPackage->total_speech_to_text_per_month + $carriedS2t;

        if (session('admin_customer') == true  && $activeNow && $oldSubscription) {
            $oldSubscription->subscription_status = 2;
            $oldSubscription->end_date = date('Y-m-d');
        }

        ...
        if (Schema::hasColumn('users', 'this_month_used_words')) {
            ...
            foreach ($users as $user) {
                if (optional(activePackageHistory())->subscription_package_id != null) {
                    $subscriptionHistory = SubscriptionHistory::latest()->where('subscription_package_id', optional(activePackageHistory())->subscription_package_id)->where('user_id', $user->id)->first();
